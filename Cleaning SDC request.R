...
ipo$First_CRSP_date_cusip <- as.Date(crsp$date[match])

match <- match(ipo$Permno_ncusip, crsp$PERMNO)
ipo$First_CRSP_date_ncusip <- as.Date(crsp$date[match])

match <- match(ipo$Permno_ncusip6, crsp$PERMNO)
ipo$First_CRSP_date_ncusip6 <- as.Date(crsp$date[match])

ipo[, Permno := -999]
ipo[, dif := First_CRSP_date_ncusip - Issue_date]
ipo[ (dif > -1 & dif <= 5) & (Permno == - 999), Permno := Permno_ncusip] 

ipo[, dif := as.numeric(as.character(First_CRSP_date_cusip - Issue_date))]
ipo[ dif > -1  & dif <= 5 &  (Permno == - 999), Permno := Permno_cusip] 

ipo[, dif := First_CRSP_date_ncusip6 - Issue_date]
ipo[ (dif > -1 & dif <= 5) & (Permno == - 999), Permno := Permno_ncusip6]

match <- match(ipo$Permno, crsp$PERMNO)
ipo$First_CRSP_date <- as.Date(crsp$date[match])
ipo$Close_price1 <- abs(crsp$PRC[match])
ipo$Close_price2 <- abs(crsp$PRC[match + 1])
ipo <- ipo[!is.na(Permno) & Permno != -999]

ipo$First_CRSP_date_cusip <- NULL
ipo$First_CRSP_date_ncusip <- NULL
ipo$First_CRSP_date_ncusip6 <- NULL
ipo$Permno_cusip <- NULL
ipo$Permno_ncusip <- NULL
